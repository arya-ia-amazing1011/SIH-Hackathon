...
import { Search, Sliders, Key, FileText, Activity, Layers, Sparkles, TrendingUp, BookOpen, GraduationCap, Shield } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="border-b border-slate-700/50 bg-[#0F172A] px-3 sm:px-5 py-2 sticky top-0 z-30 backdrop-blur-md space-y-2">
      {/* Top row: Brand, Ticker search, Mode toggles, Action buttons */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-4 flex-1 min-w-0">
          ...
          <div className="relative w-40 sm:w-52 lg:w-60">
            ...
            {isSearchOpen && searchQuery.trim() && (
              <div
                className="absolute left-0 top-full mt-1 w-full rounded-md border border-slate-700 bg-[#1E293B] shadow-2xl z-50 overflow-hidden"
              ...
              >
                {filteredTickers.map(t => (
                  <button
                    ...
                    className="flex w-full items-center justify-between px-3 py-2 text-left hover:bg-slate-700/60 transition text-xs font-mono"
                  >
                    <div>
                      <span className="font-bold text-[#06B6D4]">{t.symbol}</span>
                      <span className="ml-2 text-slate-300 font-sans">{t.name}</span>
                    </div>
            ...
            {POPULAR_TICKERS.slice(0, 5).map(t => {
              ...
              return (
                <button
                  ...
                  className={`px-2 py-0.5 text-[11px] font-bold font-mono rounded transition ${
                    isSelected
                      ? 'bg-[#06B6D4]/15 border border-[#06B6D4]/50 text-[#06B6D4] shadow-sm'
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            ...
            <span className="hidden xl:inline text-[11px] font-bold">AI ENGINE ACTIVE</span>
      ...
      {/* Primary Module Navigation Tabs (Supports toggling Monte Carlo as primary mode) */}
      <div className="flex items-center gap-1 border-t border-slate-800/80 pt-1.5 overflow-x-auto scrollbar-none font-mono text-xs">
        <button
