...
import { TrendingUp, TrendingDown, Info, Layers, DollarSign, Activity } from 'lucide-react';
...
export const TickerSummaryBar: React.FC<TickerSummaryBarProps> = ({ ticker, mode }) => {
  ...
  return (
    <div className="rounded-xl border border-slate-700/70 bg-[#1E293B] p-3.5 sm:p-4 shadow-lg text-slate-200">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 lg:gap-5 text-xs font-mono">
          ...
          <div className="flex flex-col justify-center">
            <div className="flex justify-between text-[10px] text-slate-400 mb-1">
              <span>{mode === 'pro' ? '52W L' : 'Low'}: {ticker.low52.toFixed(1)}</span>
              <span>{mode === 'pro' ? '52W H' : 'High'}: {ticker.high52.toFixed(1)}</span>
            </div>
      ...
      {/* Mode Narrative Description */}
      <div className="mt-2.5 pt-2.5 border-t border-slate-700/60 flex items-start gap-2 text-xs text-slate-300">
